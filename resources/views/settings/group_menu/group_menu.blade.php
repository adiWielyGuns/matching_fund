        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="myLargeModalLabel">Create Group Menu</h5>
                    ...
                    <div class="row">
                        <div class="col-sm-12 parent">
                            <div class="form-group">
                                <label for="name" class="col-form-label">Name Group Menu {{ dot() }}</label>
                        ...
                        <div class="col-sm-12 parent">
                            <div class="form-group">
                                <label for="icon" class="form-label">Icon Menu {{ dot() }}</label>
                                <div class="input-group">
                                    <div class="input-group-prepend bg-custom b-0">
                                        <span class="input-group-text" id="append-icon"><i
                                                class="mdi mdi-calendar"></i></span>
                                    </div>
                                    <input type="text" class="form-control required" name="icon"
                                        placeholder="Select Icon" id="icon">
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-12 parent">
                            <div class="form-group">
                                <label for="title_menu_id" class="form-label">Title Menu {{ dot() }}</label>
                                <select id="title_menu_id" name="title_menu_id" class="select2 required form-control">
                                    <option value="">Pilih Title</option>
                                    @foreach (\App\Models\TitleMenu::where('status', true)->get() as $item)
                        ...
                        <div class="col-sm-12 parent">
                            <div class="form-group">
                                <label for="type" class="form-label">Jenis Menu {{ dot() }}</label>
                                <select id="type" name="type" class="select2 required form-control">
                        ...
                        <div class="col-sm-12 parent">
                            <div class="form-group">
                                <label for="slug" class="form-label">Slug {{ dot() }}</label>
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#datatable-buttons').DataTable({
                ...
                ajax: {
                    url: "{{ route('datatable-group-menu') }}",
                    data: {
                        _token: '{{ csrf_token() }}'
                    }
                },
        ...
        $('#icon').keyup(function() {
            var html = '<i class="' + $(this).val() + '"></i>';
            console.log(html);
            $('#append-icon').html(html);
        })

        function gantiStatus(param, id) {
            $.ajax({
                url: '{{ route('ganti-status-group-menu') }}',
                data: {
                    id,
                    param
                },
        ...
        function gantiSequence(id, par) {
            var sequence = $(par).val();
            $.ajax({
                url: "{{ route('sequence-group-menu') }}",
                data: {
                    id,
                    sequence
                },
