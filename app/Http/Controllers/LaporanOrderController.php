...
class LaporanOrderController extends Controller
{
    ...
    {
        return view('laporan/laporan_order/laporan_order');
    }
    ...
    {
        $data = Order::where(function ($q) use ($req) {
        ...
        return DataTables::of($data)
            ...
            ->addColumn('table', function ($data) {
                return $data->table ? $data->table->name : '-';
            })
            ->addColumn('is_reservation', function ($data) {
                if ($data->jenis == 'reservasi') {
                    return '<span class="badge badge-pill badge-primary">Ya</span>';
                } else {
                    return '<span class="badge badge-pill badge-pink text-light">Tidak</span>';
                }
            })
            ...
            ->rawColumns(['aksi', 'status', 'icon', 'sequence', 'is_reservation'])
            ->addIndexColumn()
            ->make(true);
    }
